...
# Usage: python pwr_measure.py cpu_600 60 # for cpu measurements, 600MHz, 60secs
# Usage: python pwr_measure.py perf_1200 60 # for performance counter measurements, 1200MHz, 60secs
import sys
import os
...
if len(sys.argv) > 1:
    version = sys.argv[1]
    secs = int(sys.argv[2])
elapsed_time_in_ms = secs * 1000 # seconds

filename = None
if version is not None:
	filename = "./pwr_" + version + ".txt"
else:
	filename = "./pwr_" + datetime.datetime.now().strftime("%H%M%S%m%d%Y") + \
		".txt"
f = open(filename, "w")
